function computeBMI(weight, height) {
  ...
  if (!Number.isFinite(w) || !Number.isFinite(h) || w <= 0 || h <= 0) {
    throw Object.assign(
        new Error('Некорректные параметры: вес и рост должны быть положительными числами'),
        { status: 400 }
    );
  }
  ...
  if (bmi < 18.5) {
    ...
    advice = 'Добавьте калорийность, включите силовые тренировки и наблюдайтесь у врача.';
  } else if (bmi < 25) {
    category = 'Норма';
    advice = 'Поддерживайте баланс питания и активности.';
  } else if (bmi < 30) {
    category = 'Избыточная масса';
    advice = 'Сократите калорийность и увеличьте активность, обсудите план с врачом.';
  } else {
    category = 'Ожирение';
    advice = 'Обратитесь к специалисту, составьте программу снижения массы и наблюдайтесь.';
  }
...
export function getBmiInfo(req, res) {
  res.json({
    ...
    examples: [
      '/api/bmi/calc?weight=70&height=170',
      '/api/bmi/calc?weight=70&height=1.70',
      '/api/bmi/demo/70/170'
    ]
  });
}

export function calcFromParams(req, res, next) {
  try {
